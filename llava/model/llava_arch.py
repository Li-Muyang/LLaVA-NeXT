...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images):
        ...
        if getattr(self.config, "use_cka_loss", False):
            # Store flattened vision features for CKA computation
            # image_features shape: (num_images * num_patches, hidden_dim) or (num_images, num_patches, hidden_dim)
            if image_features.dim() == 3:
                self._cached_vision_features = image_features.reshape(-1, image_features.shape[-1])
            else:
                self._cached_vision_features = image_features
    ...
    def prepare_inputs_labels_for_multimodal(self, input_ids, position_ids, attention_mask, past_key_values, labels, images, modalities=["image"], image_sizes=None):
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if getattr(self.config, "use_cka_loss", False) and self.training:
                if not hasattr(self, '_cached_language_features'):
                    self._cached_language_features = []
                self._cached_language_features.append(cur_input_embeds.reshape(-1, cur_input_embeds.shape[-1]))
        ...
        if getattr(self.config, "use_cka_loss", False):
            ...
            if hasattr(self, '_cached_language_features') and len(self._cached_language_features) > 0:
                self._cached_language_features = torch.cat(self._cached_language_features, dim=0)
            else:
                self._cached_language_features = None
